...
public class ReporteServiceImpl implements IReporteService {
    ...
    public ReporteRespuestaDTO getProductosMasVendidos(ReporteFiltrosDTO filtros, String username, String rol) {
        StringBuilder sql = new StringBuilder(
            ...
            "JOIN inventario.lote l ON dv.id_lote = l.id_lote " +
    ...
    public ReporteRespuestaDTO getProductosMayorGanancia(ReporteFiltrosDTO filtros) {
        StringBuilder sql = new StringBuilder(
            ...
            "SUM((dv.precio_unitario - l.costo_unitario_real) * dv.cantidad) AS ganancia_total " +
            "FROM operaciones.detalle_ventas dv " +
            "JOIN inventario.lote l ON dv.id_lote = l.id_lote " +
            "JOIN inventario.producto p ON l.id_producto = p.id_producto " +
            "JOIN operaciones.ventas v ON v.id_venta = dv.id_venta " +
            "WHERE 1=1 "
        );
        List<Object> params = new ArrayList<>();
    ...
    public ReporteRespuestaDTO getProductoMayorDemandaTemporada(ReporteFiltrosDTO filtros) {
        ...
        String sql = "SELECT p.nombre AS producto, t.nombre AS temporada, SUM(dv.cantidad) AS cantidad_vendida " +
                     ...
                     "JOIN inventario.lote l ON dv.id_lote = l.id_lote " +
                     "JOIN inventario.producto p ON l.id_producto = p.id_producto " +
                     "JOIN operaciones.temporada t ON v.fecha_venta >= t.fecha_inicio " +
                     "  AND (t.fecha_fin IS NULL OR v.fecha_venta <= t.fecha_fin) " +
    ...
    public ReporteRespuestaDTO getDiaMasCompras(ReporteFiltrosDTO filtros) {
        String sql = "SELECT to_char(fecha_venta, 'Day') AS dia_semana, COUNT(id_venta) AS total_ventas " +
    ...
    public ReporteRespuestaDTO getRotacionTemporada(ReporteFiltrosDTO filtros, String username, String rol) {
        StringBuilder sql = new StringBuilder(
            "SELECT p.nombre AS producto, AVG(v.fecha_venta - l.fecha_recepcion) AS dias_rotacion_promedio " +
            "FROM operaciones.detalle_ventas dv " +
            "JOIN operaciones.ventas v ON dv.id_venta = v.id_venta " +
            "JOIN inventario.lote l ON dv.id_lote = l.id_lote " +
            "JOIN inventario.producto p ON l.id_producto = p.id_producto " +
            "WHERE l.fecha_recepcion IS NOT NULL "
        );
        List<Object> params = new ArrayList<>();
    ...
    public ReporteRespuestaDTO getEfectividadCombos(ReporteFiltrosDTO filtros, String username, String rol) {
        StringBuilder sql = new StringBuilder(
            "SELECT u.nombres || ' ' || u.apellidos AS supervisor, " +
            ...
            "JOIN seguridad.usuarios u ON pr.id_usuario_aprueba = u.id_usuario " +
        ...
        sql.append(" GROUP BY u.nombres, u.apellidos ORDER BY ventas_generadas DESC");
    ...
    public ReporteRespuestaDTO getArticulosEstancados(ReporteFiltrosDTO filtros) {
        ...
        String sql = "SELECT p.nombre AS producto, l.numero_lote AS lote, MAX(v.fecha_venta) AS ultima_venta " +
                     "FROM inventario.lote l " +
                     "JOIN inventario.producto p ON l.id_producto = p.id_producto " +
                     "LEFT JOIN operaciones.detalle_ventas dv ON l.id_lote = dv.id_lote " +
                     "LEFT JOIN operaciones.ventas v ON dv.id_venta = v.id_venta " +
                     "GROUP BY p.nombre, l.numero_lote " +
                     "HAVING MAX(v.fecha_venta) < CURRENT_DATE - INTERVAL '60 days' OR MAX(v.fecha_venta) IS NULL " +
    ...
    public ReporteRespuestaDTO getTotalInventarioAlmacenado(ReporteFiltrosDTO filtros, String rol) {
        ...
        String select = mostrarMontos 
            ? "SELECT p.nombre AS producto, SUM(l.cantidad_actual) AS total_unidades, SUM(l.cantidad_actual * l.costo_unitario_real) AS valor_total " 
    ...
    public ReporteRespuestaDTO getMovimientosPorProducto(ReporteFiltrosDTO filtros) {
        StringBuilder sql = new StringBuilder(
            ...
            "JOIN inventario.lote l ON m.id_lote = l.id_lote " +
            "JOIN inventario.producto p ON l.id_producto = p.id_producto " +
            "WHERE 1=1 "
        );
        List<Object> params = new ArrayList<>();
    ...
    public ReporteRespuestaDTO getAlertasCriticas(ReporteFiltrosDTO filtros) {
        String sql = "SELECT l.numero_lote AS lote, ac.riesgo_evaluado AS riesgo, ac.fecha_alerta AS fecha " +
                     "FROM ia_alertas.alerta_caducidad ac " +
                     "JOIN inventario.lote l ON ac.id_lote = l.id_lote " +
                     "WHERE ac.id_estado = 1 " + // ACTIVA
                     "ORDER BY ac.fecha_alerta DESC";
        List<Map<String, Object>> data = jdbcTemplate.queryForList(sql);
    ...
    public ReporteRespuestaDTO getOrdenesPendientesDespacho(ReporteFiltrosDTO filtros, String username, String rol) {
        String sql = "SELECT o.numero_orden AS orden, p.nombre_razon_social AS proveedor, o.fecha_esperada AS esperada " +
    ...
    public ReporteRespuestaDTO getOrdenesDespachadasHoy(ReporteFiltrosDTO filtros, String username, String rol) {
        StringBuilder sql = new StringBuilder(
            "SELECT u.nombres || ' ' || u.apellidos AS bodeguero, COUNT(DISTINCT m.id_orden_compra) AS despachos " +
            "FROM operaciones.movimientos_inventario m " +
            "JOIN seguridad.usuarios u ON m.id_usuario = u.id_usuario " +
            "WHERE m.tipo_movimiento = 'SALIDA' AND m.fecha_movimiento::date = CURRENT_DATE "
        );
        List<Object> params = new ArrayList<>();
    ...
    public ReporteRespuestaDTO getClientesMasCompran(ReporteFiltrosDTO filtros, String username, String rol) {
        StringBuilder sql = new StringBuilder(
            "SELECT c.nombres || ' ' || c.apellidos AS cliente, SUM(v.total) AS total_comprado " +
            "FROM operaciones.ventas v " +
        ...
        sql.append(" GROUP BY cliente ORDER BY total_comprado DESC");
    ...
    public ReporteRespuestaDTO getMercaderiaDevueltaPorMes(ReporteFiltrosDTO filtros, String username, String rol) {
        StringBuilder sql = new StringBuilder(
            "SELECT to_char(d.fecha_devolucion, 'YYYY-MM') AS mes, d.motivo_devolucion AS motivo, COUNT(*) AS cantidad " +
        ...
        sql.append(" GROUP BY mes, d.motivo_devolucion ORDER BY mes DESC, cantidad DESC");
    ...
    public ReporteRespuestaDTO getClientesChurn(ReporteFiltrosDTO filtros, String username, String rol) {
        ...
        StringBuilder sql = new StringBuilder(
            "SELECT c.nombres || ' ' || c.apellidos AS cliente, MAX(v.fecha_venta) AS ultima_compra " +
        ...
        sql.append(" GROUP BY cliente HAVING MAX(v.fecha_venta) < CURRENT_DATE - INTERVAL '90 days'");
