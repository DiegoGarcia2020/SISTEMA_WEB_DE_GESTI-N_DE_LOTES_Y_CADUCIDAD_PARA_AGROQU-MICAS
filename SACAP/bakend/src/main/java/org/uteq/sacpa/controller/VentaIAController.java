...
/**
 * Módulo 3 — Ventas del Técnico de Campo (Motor IA).
 *
...
public class VentaIAController {

    private final IVentaIAService ventaService;
    ...
    @PostMapping
    public ResponseEntity<VentaIAResponseDTO> crearVenta(@Valid @RequestBody VentaCreateRequestDTO request) {
        return ResponseEntity.status(201).body(ventaService.crearVenta(idUsuarioAutenticado(), request));
    }

    @GetMapping
    public ResponseEntity<List<VentaIAResponseDTO>> misVentas() {
