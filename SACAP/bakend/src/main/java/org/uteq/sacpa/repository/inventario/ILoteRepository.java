...
public interface ILoteRepository extends JpaRepository<Lote, Integer> {
    ...
    @Query("SELECT l FROM Lote l WHERE l.producto.idEstado = 1 AND l.idEstadoLote = 1 AND l.fechaVencimiento > CURRENT_DATE AND (l.cantidadActual - COALESCE(l.cantidadReservada, 0)) > 0 ORDER BY l.fechaVencimiento ASC")
    List<Lote> findLotesDisponiblesFefo();

    @Query("SELECT l FROM Lote l JOIN l.producto p WHERE l.producto.idEstado = 1 AND l.idEstadoLote = 1 AND l.fechaVencimiento > CURRENT_DATE AND (l.cantidadActual - COALESCE(l.cantidadReservada, 0)) > 0 AND (:busqueda IS NULL OR LOWER(p.nombre) LIKE LOWER(CONCAT('%', :busqueda, '%')) OR LOWER(l.numeroLote) LIKE LOWER(CONCAT('%', :busqueda, '%'))) ORDER BY l.fechaVencimiento ASC")
