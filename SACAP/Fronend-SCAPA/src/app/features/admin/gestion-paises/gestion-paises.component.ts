...
import { PaisDTO } from '../../../core/models/geografia.model';
...
@Component({
  ...
  template: `
    <div class="space-y-6 animate-fade-in pb-12">
      <app-section-header title="Gestion de Paises" 
                          subtitle="Administra los registros de paises del sistema SACPA.">
      ...
      <div class="bg-white rounded-2xl border border-gray-200/80 shadow-xs p-4">
        <div class="flex items-center gap-3">
          <div class="flex-1 relative">
            ...
            <input type="text" [(ngModel)]="searchQuery" placeholder="Buscar pais por nombre o codigo ISO..."
          ...
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="bg-gray-50 border-b border-gray-200/80 text-[11px] font-bold text-gray-500 uppercase tracking-wider">
                ...
                <th class="py-4 px-6">Codigo ISO</th>
                <th class="py-4 px-6 text-right">Acciones</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-100 text-sm">
              @for (p of paginatedItems(); track p.idPais) {
                <tr class="hover:bg-green-50/20 transition-colors group">
                  <td class="py-4 px-6 font-mono font-bold text-gray-800">
                    <span class="bg-gray-100 border border-gray-200 px-2.5 py-1 rounded-md text-xs">{{ p.idPais }}</span>
                  </td>
                  <td class="py-4 px-6 font-extrabold text-gray-900 group-hover:text-[#0B4628] transition-colors">{{ p.nombre }}</td>
                  <td class="py-4 px-6 font-mono text-xs text-gray-600">{{ p.codigoIso }}</td>
                ...
                <tr class="text-center text-gray-400">
                  <td colSpan="4" class="py-12 text-xs font-medium">No se encontraron paises</td>
                </tr>
              }
            </tbody>
          </table>
        ...
        <div class="fixed inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center z-50 animate-fade-in p-4">
          <div class="bg-white rounded-2xl shadow-xl border border-gray-200 max-w-md w-full overflow-hidden">
            <div class="p-6 border-b border-gray-100 flex items-center justify-between bg-green-50/60">
              <div class="flex items-center gap-2">
                <lucide-icon name="globe" class="w-5 h-5 text-[#0B4628]"></lucide-icon>
                <h3 class="font-bold text-base text-gray-900">{{ editingId() ? 'Editar Pais' : 'Registrar Nuevo Pais' }}</h3>
              </div>
            ...
            <div class="p-6 space-y-4">
              <div>
                <label class="block text-xs font-bold text-gray-700 uppercase mb-1">Nombre del Pais</label>
                <input type="text" [(ngModel)]="form.nombre" placeholder="Ej: Ecuador"
                       class="w-full px-3.5 py-2 border border-gray-300 rounded-xl text-sm font-bold focus:border-[#0B4628] outline-none">
              </div>

              <div>
                <label class="block text-xs font-bold text-gray-700 uppercase mb-1">Codigo ISO</label>
                <input type="text" [(ngModel)]="form.codigoIso" placeholder="Ej: EC" maxlength="10"
                       class="w-full px-3.5 py-2 border border-gray-300 rounded-xl text-sm font-mono uppercase focus:border-[#0B4628] outline-none">
              </div>
            </div>
...
export class GestionPaisesComponent implements OnInit {
  private geoService = inject(GeografiaService);
  private toast = inject(ToastService);

  items = signal<PaisDTO[]>([]);
  searchQuery = '';
  isModalOpen = signal<boolean>(false);
  editingId = signal<number | null>(null);
  form = { nombre: '', codigoIso: '' };
  ...
  loadData(): void {
    this.geoService.listarPaises().subscribe(data => this.items.set(data));
  }
  ...
  filteredItems = computed(() => {
    ...
    return this.items().filter(p =>
      !q || p.nombre.toLowerCase().includes(q) || p.codigoIso.toLowerCase().includes(q)
    );
  });
  ...
  openCreateModal(): void {
    ...
    this.form = { nombre: '', codigoIso: '' };
    this.isModalOpen.set(true);
  }

  openEditModal(pais: PaisDTO): void {
    this.editingId.set(pais.idPais);
    this.form = { nombre: pais.nombre, codigoIso: pais.codigoIso };
    this.isModalOpen.set(true);
  }

  saveItem(): void {
    if (!this.form.nombre.trim() || !this.form.codigoIso.trim()) {
      this.toast.warning('Validacion', 'Ingresa el nombre y el codigo ISO del pais.');
      return;
    }

    if (this.editingId()) {
      this.geoService.actualizarPais(this.editingId()!, this.form).subscribe({
        next: () => {
          this.toast.success('Pais actualizado', `"${this.form.nombre}" ha sido actualizado.`);
          this.isModalOpen.set(false);
          this.loadData();
        }
      });
    } else {
      this.geoService.crearPais(this.form).subscribe({
        next: () => {
          this.toast.success('Pais registrado', `"${this.form.nombre}" ha sido creado.`);
          this.isModalOpen.set(false);
          this.loadData();
        }
      });
  ...
  deleteItem(pais: PaisDTO): void {
    if (confirm(`Desactivar el pais "${pais.nombre}"?`)) {
      this.geoService.desactivarPais(pais.idPais).subscribe({
        next: () => {
          this.toast.info('Pais desactivado', `"${pais.nombre}" ha sido desactivado.`);
          this.loadData();
        }
      });
    }
